...
const customerNavItems: NavItem[] = [
  { name: "Dashboard", icon: <GridIcon />, path: "/" },
  {
    ...
    subItems: [
      ...
      { name: "My Policies", path: "/policies" },
      { name: "Buy Insurance", path: "/policies/buy" },
...
const officerNavItems: NavItem[] = [
  {
    ...
    icon: <PageIcon />,
    subItems: [{ name: "Assigned Claims", path: "/officer/claims" }],
  },
];
...
const AppSidebar: React.FC = () => {
  ...
  // Select which navigation to show based on role
    ...
    >
      ...
      >
        <Link to="/">
          {expanded ? (
            <>
              <img src="/images/logo/insureai-logo-light.png" alt="InsureAI" className="block h-12 w-auto object-contain dark:hidden" />
              <img src="/images/logo/insureai-logo-dark.png" alt="InsureAI" className="hidden h-12 w-auto object-contain dark:block" />
            </>
          ) : (
            <img src="/images/logo/insureai-logo-icon.png" alt="InsureAI" className="h-10 w-10 object-contain" />
          )}
        </Link>
      </div>
      ...
      <div className="flex-1 overflow-y-auto py-7 no-scrollbar">
        ...
        {expanded && user && (
          <div className="mt-8 rounded-2xl bg-gradient-to-br from-blue-600 to-cyan-500 p-4 text-white shadow-sm">
            ...
            <p className="mt-1 text-xs leading-5 text-blue-50">{user.full_name}</p>
