...
export default function VerifyEmail() {
  ...
  // Email comes from the URL query param set by SignUpForm after registration
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      setCountdown(60); // wait 60s before resending again
      setCodes(["", "", "", "", "", ""]);
      inputRefs.current[0]?.focus();
    } catch (err) {
    ...
    <>
      ...
      <div className="flex min-h-screen items-center justify-center bg-gray-50 px-4 dark:bg-gray-950">
        <div className="w-full max-w-md">
          ...
          {/* Dev hint — only visible when no SMTP configured */}
          <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-center dark:border-amber-500/20 dark:bg-amber-500/5">
            <p className="text-xs text-amber-700 dark:text-amber-400">
              <strong>Development mode:</strong> If email is not configured, the
              verification code is printed in the backend server console.
            </p>
          </div>
        </div>
      </div>
